...
function atomWithQuery(createQueryArgs, getClient = (get) => get(clientAtom)) {
  const refAtom = atom(() => ({}));
  const createResultAtom = (ref, client, args, opts) => {
    let resolve = null;
    ...
    resultAtom.onMount = (update) => {
      setResult = update;
      const subscription = pipe(client.query(args.query, args.variables, __spreadValues(__spreadValues(__spreadValues({}, args.requestPolicy && { requestPolicy: args.requestPolicy }), args.context), opts)), skip(1), subscribe(listener));
